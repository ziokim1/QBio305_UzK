...
window_start <- seq(from = polh_start, to = chr, by = window_jump)
...
plot(density(log(ara_data$SWE_pi)), col= "blue", main="D", xlim=c(-8, -3.5))
...
# Add Kruskal-Wallis test p-value to the plot
a_td2 + annotate("text", size = 6, x = 1.5, y = -1.5, label = paste("Kruskal-Wallis p =", format.pval(kruskal_test_result$p.value, digits = 3)))
...
# modify code change xlim to see complete distribution
plot(density(subset_ITA_td), col = "red", main = "D", xlim = c(-12.5,-1.5))
